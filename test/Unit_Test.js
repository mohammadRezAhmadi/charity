      ...
      beforeEach(async function () {
        ...
        it("Intro Needay in contract!" , async function(){
          //introNeeded
            ...
            console.log(`Examination of the needy: ${await charity.needy(0)} + ${await charity.needy(1)} + ${await charity.needy(2)}`);
            console.log(` helpers information : ${await charity.helpers(0)} + ${await charity.helpers(1)} + ${await charity.helpers(2)} + ${await charity.helpers(3)}`);
            // the payment
            await charity.connect(ownerSign[3]).changeLevelToEnd();
            console.log(`Contract Balance: ${await charity.getBalance()}`);
            await charity.connect(ownerSign[4]).payOff(0);
            await charity.connect(ownerSign[5]).payOff(1);
            await charity.connect(ownerSign[6]).payOff(2);
